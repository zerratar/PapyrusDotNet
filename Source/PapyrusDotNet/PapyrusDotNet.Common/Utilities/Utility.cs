...
namespace PapyrusDotNet.Common.Utilities
{
    public class Utility : IUtility
    {
        ...
        public static string GetPapyrusReturnType(TypeReference reference, TypeDefinition caller,
            bool stripGenericMarkers)
        {
            ...
            if (stripGenericMarkers && reference.IsGenericInstance)
            {
                retType = retType.Replace("`1",
                    "_" + GetPapyrusBaseType(reference.FullName.Split('<')[1].Split('>')[0], caller));
            }
        ...
        public static string GetPapyrusReturnType(string reference, TypeDefinition caller = null)
        {
            ...
            return GetPapyrusReturnType(reference, "", caller);
        }
        ...
        {
            return value is int || value is byte || value is short || value is long || value is double || value is float ||
