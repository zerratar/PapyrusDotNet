...
namespace PapyrusDotNet
{
    ...
    class Program
    {
        ...
        public static string CreatePapyrusObjectTable(TypeDefinition type)
        {
            ...
            var mergeCtorAndOnInit = false;
            // If we contain both a constructor and a OnInit, we want to merge these two methods.
            // But before we can do that, we first need to reorder our list to handle the constructor last.
            // That way, we can just inject the logic of the constructor at the top of OnInit.
            if (ctorAvailable && onInitAvailable)
            {
                methods = methods.OrderByDescending(c => c.Name).ToList();
                mergeCtorAndOnInit = true;
            }

            foreach (var method in methods)
            {
                papyrus += CreatePapyrusMethod(type, method, mergeCtorAndOnInit, onInitAvailable, ctorAvailable);
            }
        ...
        static string CreatePapyrusMethod(TypeDefinition type, MethodDefinition method, bool mergeConstructorAndOnInit = false, bool onInitAvailable = false, bool hasConstructor = false)
        {
            // We did not support constructors earlier.
